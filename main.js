...
const checkLuhn = arr => {
    let checkIndex = arr.length - 1;
    let sum = arr[checkIndex];
...
const findInvalidCards = cards => {
    ...
    for (i = 0; i < cards.length ; i++){
        let cardInTest = cards[i];
        if (!validateCred(cardInTest)){
            invalidCards.push(cardInTest);
        }
    }
    return invalidCards;
};
...
const idInvalidCardCompanies = cards => {
    ...
    for (let i=0; i < cards.length -1; i ++) {
      invCards.push(cards[i][0]);
    }

      const uniqueSet = new Set(invCards);
